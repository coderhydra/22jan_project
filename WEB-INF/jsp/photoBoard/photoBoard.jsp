...
<head>
    ...
    <script>
    function regi(){
    	location.href="/library/photoBoard/regi";
    	return false;
    }
    function detail(page,id){
    	location.href="/library/photoBoard/detail/"+page+"/"+id;
    	return false;
    }
    function edit(page,id){
    	location.href="/library/photoBoard/edit/"+page+"/"+id;
    	return false;
    }
    function del(id,pg){
    	if(!confirm("정말로 글을 삭제하시겠어요?")) return;
    	$.ajax({
    		url:"/library/photoBoard/delete/"+id,
    		method:'get',
    		cache:false,
    		dataType:'text',
    		success: function(res){
				alert(res ? "글이 삭제되었습니다.":"글 삭제에 실패하였습니다.");
				location.href="/library/photoBoard/list/page/"+pg;
				return false;
			},
    ...
    function search(){
		...
		$.ajax({
			...
			success:function(res){
				if(res.ok){
				location.href="/library/photoboard/search/1/"+cmd+"/"+word;
				return false;
				}
				alert("검색결과가 없습니다.");
			},
